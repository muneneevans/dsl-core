...
class IndicatorGroupsIndicatorsListView(ListAPIView):
    def get(self, request, **kwargs):
        response = indicators.get_indicator_group_indicators(
            int(kwargs['indicatorgroupid']), True)
        return HttpResponse(response)
...
class DataElementDetailView(ListAPIView):
    def get(self, request, **kwargs):
        response = dataelements.get_dataelement_by_id(kwargs['dataelement_id'],
                                                      True)
...
class DataElementDataValuesListView(ListAPIView):
    def get(self, request, **kwargs):
        result = datavalues.get_dataelement_datavalues(
            kwargs['dataelement_id'], True)
...
class FacilityDataElementDataValues(ListAPIView):
    def get(self, request, **kwargs):
        response = datavalues.get_facility_dataelement_datavalues(
            kwargs['dataelement_id'], kwargs['facility_id'], True)
...
class FacilityIndicatorDataValues(ListAPIView):
    def post(self, request, **kwargs):
        if request.data:
            ...
            required_filters = [
                'facilityId', 'indicatorId', 'periodTypeId', 'year'
            ]
            ...
            response = datavalues.get_facility_indicator_datavalues(
                filters['facilityId'], filters['indicatorId'],
...
class WardIndicatorDataValues(ListAPIView):
    def post(self, request, **kwargs):
        if request.data:
            filters = request.data['filters']
            required_filters = [
                'wardId', 'indicatorId', 'periodTypeId', 'year'
            ]
            for r_filter in required_filters:
                if not r_filter in filters.keys():
                    return JsonResponse(
            ...
            response = datavalues.get_ward_indicator_datavalues(
                filters['wardId'], filters['indicatorId'],
                filters['periodTypeId'], filters['year'])
...
class ConstituencyIndicatorDataValues(ListAPIView):
    def post(self, request, **kwargs):
        if request.data:
            filters = request.data['filters']
            required_filters = [
            ...
            response = datavalues.get_constituency_indicator_datavalues(
...
class WardIndicatorFacilityDataValues(ListAPIView):
    def post(self, request, **kwargs):
        if request.data:
            ...
            response = datavalues.get_ward_facility_indicator_datavalues(
                filters['wardId'], filters['indicatorId'],
                filters['periodTypeId'], filters['year'])
...
class CategoryOptionComboDetailView(ListAPIView):
    def get(self, request, **kwargs):
        response = category_option_combos.get_category_option_combo_by_id(
            kwargs['category_option_combo_id'], True)
        return HttpResponse(response)
